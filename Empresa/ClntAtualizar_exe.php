	  ...
	  <!-- Acesso em:-->
		...
		<?php
			...
			if ($_FILES['Imagem']['size'] == 0) { // Não recebeu uma imagem binária
				$sql = "UPDATE Funcionario SET Nome = '$nome', CPF = '$CPF', Data_nasc = '$dtnasc' WHERE ID_Fun = $id";
			}else{
				$imagem = addslashes(file_get_contents($_FILES['Imagem']['tmp_name'])); // Prepara para salvar em BD
				$sql = "UPDATE Funcionario SET Nome = '$nome', CPF = '$CPF', Data_nasc = '$dtnasc', Foto = '$imagem' WHERE ID_Fun = $id";	
			}
